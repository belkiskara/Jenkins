...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABSE_URI"] = "sqlite:///./todo-list.db"
...
@app.route("/")
def home():
    return jasonify({"tasks":get_all_tasks()})
@app.route("/todos", methods=["GET"])
def get_tasks():
    return "This returns all tasks"
@app.route("/todos/<int:task_id>", methods=["GET"])
def get_task(task_id):
    task = find_task(task_id)
    if task == None:
        abort(404)
    return jsonify({"task_found": task})
@app.route("/todos", methods=["POST"])
def add_task():
    if not request.json or not "title" in equest.json:
        abort(400)
...
def update_task(task_id):
    ...
    return jsonify({"updated task": change_task(tasks)})
